...
\begin{figure}[ht]
    ...
    \begin{minipage}{0.49\textwidth}
        ...
        \begin{algorithmic}[1]
            \footnotesize
            \Require $a \in \mathbb{Z}_p[x]/( x^N + 1 )$
            \Require Raiz $\psi$ de ordem $2N$
    ...
    \begin{minipage}{0.49\textwidth}
        \hrule
        \vskip 2pt
        \captionof{algorithm}{NTT Inversa (GS)}
        \hrule
        \begin{algorithmic}[1]
            \footnotesize
            \Require $\hat{a}$ (\emph{bit-reversed})
            \Require Raiz $\psi$ de ordem $2N$
            ...
            \For{$l \gets 0$ \textbf{to} $m-1$}
                ...
                \EndFor
            \EndFor
            \State $invN \gets \text{inv}(N) \pmod p$
            \For{$i \gets 0$ \textbf{to} $N-1$}
                \State $a[i] \gets a[i] \cdot invN$
            \EndFor
            \State \Return $a$ (\emph{normal order})
        \end{algorithmic}
        \hrule
    \end{minipage}
\end{figure}
