...
Para viabilizar o processamento em computadores, utilizamos a \textbf{DFT}. Ela opera sobre uma sequência finita de $N$ amostras, mapeando dados discretos no tempo para dados discretos na frequência, devido ao teorema de Teorema de amostragem de Nyquist-Shannon[1colocar uma referencia], um sinal de frequência $f$ precisa de $N \geq f/2$ para ser unicamente determinado.
...
A eficiência da NTT reside na estratégia ``dividir para conquistar''. Em vez de avaliar um polinômio de grau $N-1$ de uma só vez, utilizamos a estrutura algébrica do anel para decompor o problema em dois problemas independentes de tamanho $N/2$.

\subsubsection*{1. A Fatoração do Módulo}
O ponto de partida é a decomposição do polinômio $X^N + 1$.
Como $\psi$ é uma raiz primitiva $2N$-ésima, temos que $\psi^N \equiv -1$. Isso implica que o termo $\psi^{N/2}$ atua como uma ``raiz quadrada de -1'' no corpo finito.

Podemos então ver $X^N + 1$ como uma diferença de quadrados:
\[
X^N + 1 = X^N - (\psi^{N/2})^2 = (X^{N/2} - \psi^{N/2}) \cdot (X^{N/2} + \psi^{N/2})
\]
O Teorema Chinês dos Restos (CRT) garante que resolver o problema no anel original é equivalente a resolvê-lo simultaneamente nestes dois sub-anéis menores.

\subsubsection*{2. O Colapso da Variável}
Considere o polinômio de entrada $A(X)$ dividido em sua metade inferior ($A_{lo}$) e superior ($A_{hi}$):
\[
A(X) = \sum_{n=0}^{N/2-1} x[n]X^n + X^{N/2} \sum_{n=0}^{N/2-1} x[n+N/2]X^n
\]
A intuição chave da NTT é que, ao projetarmos $A(X)$ nos sub-anéis definidos pelos fatores acima, a potência $X^{N/2}$ deixa de ser uma variável e colapsa em uma constante:

\begin{itemize}
    \item \textbf{No ramo esquerdo} (módulo $X^{N/2} - \psi^{N/2}$): Temos a equivalência $X^{N/2} \equiv \psi^{N/2}$.
    \item \textbf{No ramo direito} (módulo $X^{N/2} + \psi^{N/2}$): Temos a equivalência $X^{N/2} \equiv -\psi^{N/2}$.
\end{itemize}

Isso simplifica drasticamente o cálculo, pois reduz a multiplicação polinomial a somas ponderadas.

\subsubsection*{3. A Operação Borboleta (Butterfly)}
Substituindo as equivalências acima na expressão de $A(X)$, obtemos as fórmulas para os novos polinômios $A_L(X)$ e $A_R(X)$ de grau $N/2 - 1$. Em termos dos coeficientes, para cada $0 \le i < N/2$:

\[
\boxed{
\begin{aligned}
    x_L[i] &= x[i] + \psi^{N/2} \cdot x[i+N/2] \pmod p \\
    x_R[i] &= x[i] - \psi^{N/2} \cdot x[i+N/2] \pmod p
\end{aligned}
}
\]

Esta é a clássica operação ``borboleta''. Observe a simetria: os mesmos valores de entrada ($x[i]$ e $x[i+N/2]$) são usados para calcular ambas as saídas, diferenciando-se apenas pelo sinal da operação.

O processo é então aplicado recursivamente: $x_L$ torna-se a entrada para uma NTT de tamanho $N/2$, e $x_R$ para outra, até que o tamanho do problema seja trivial (1).
...
Enquanto a transformação direta (Forward NTT) utiliza a estratégia de decomposição para reduzir o grau do polinômio, a transformação inversa (INTT) realiza o caminho oposto: ela combina pares de polinômios menores para reconstruir o polinômio original modulo $X^N + 1$. O algoritmo de Gentleman-Sande implementa essa reconstrução de forma eficiente.

\subsubsection*{1. O Problema da Interpolação}
Na etapa inversa, começamos com dois polinômios "metade" recuperados dos sub-anéis:
\begin{itemize}
    \item $A_L(X)$: O resultado associado ao ramo $X^{N/2} - \psi^{N/2}$.
    \item $A_R(X)$: O resultado associado ao ramo $X^{N/2} + \psi^{N/2}$.
\end{itemize}
Nosso objetivo é encontrar o polinômio único $A(X) = A_{lo}(X) + X^{N/2}A_{hi}(X)$ que satisfaz:
\[
\begin{cases}
    A(X) \equiv A_L(X) \pmod{X^{N/2} - \psi^{N/2}} \\
    A(X) \equiv A_R(X) \pmod{X^{N/2} + \psi^{N/2}}
\end{cases}
\]

\subsubsection*{2. A Lógica da Recuperação}
Lembrando que no sub-anel esquerdo $X^{N/2} \equiv \psi^{N/2}$ e no direito $X^{N/2} \equiv -\psi^{N/2}$, podemos escrever o sistema linear para os componentes $A_{lo}$ e $A_{hi}$:

\[
\begin{aligned}
    A_L &= A_{lo} + \psi^{N/2} A_{hi} \\
    A_R &= A_{lo} - \psi^{N/2} A_{hi}
\end{aligned}
\]

Para isolar $A_{lo}$ e $A_{hi}$, basta somar e subtrair as equações:
\begin{itemize}
    \item \textbf{Soma:} $A_L + A_R = 2A_{lo} \implies A_{lo} = \frac{1}{2}(A_L + A_R)$
    \item \textbf{Subtração:} $A_L - A_R = 2\psi^{N/2} A_{hi} \implies A_{hi} = \frac{1}{2}\psi^{-N/2}(A_L - A_R)$
\end{itemize}

Esta estrutura algébrica revela a diferença fundamental entre a borboleta da ida (Cooley-Tukey) e a da volta (Gentleman-Sande).

\subsubsection*{3. A Borboleta GS (Inverse Butterfly)}
Diferente da NTT direta, onde a multiplicação pelo fator de rotação ocorre \textit{antes} da borboleta (ou no ramo inferior da entrada), na INTT via Gentleman-Sande a multiplicação ocorre \textit{após} a subtração.

Para coeficientes de entrada $y_L$ e $y_R$ correspondentes à mesma posição nos vetores dos sub-blocos, a operação de fusão é:

\[
\boxed{
\begin{aligned}
    x[i] &= \frac{1}{2} (y_L[i] + y_R[i]) \pmod p \\
    x[i+N/2] &= \frac{1}{2} (y_L[i] - y_R[i]) \cdot \psi^{-N/2} \pmod p
\end{aligned}
}
\]
\textit{Nota: Em implementações eficientes, o fator escalar $1/2$ (inverso modular de 2) é frequentemente acumulado e aplicado apenas uma vez no final como $1/N$, para evitar multiplicações excessivas a cada estágio.}

\subsubsection*{4. Fluxo de Dados}
O algoritmo Gentleman-Sande é a "transposta" do Cooley-Tukey.
Se a NTT direta recebe os dados em ordem natural e os devolve em ordem \textit{bit-reversed} (embaralhada), a INTT via GS aceita os dados em ordem \textit{bit-reversed} e os reconstrói para a ordem natural. Isso elimina a necessidade de reordenamentos custosos de memória entre a transformação direta e a inversa durante operações de convolução.
