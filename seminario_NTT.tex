...
Como a IDFT é praticamente identica igual à DFT, podemos usar o mesmo algoritmo da FFT para calcular a IFFT
\begin{algorithm}
...
Solução: utilizar uma transformada que utiliza apenas números exatos e realiza uma convolução cíclica
...
A discussão de \emph{bit-reversal} depende da variante (DIT/DIF) e do arranjo em memória (implementação iterativa in-place).
Em muitas implementações práticas, usa-se o par:
\[
\text{(DIT) CT}:\ \text{input natural} \to \text{saída permutada}
\qquad\text{e}\qquad
\text{(DIF) GS}:\ \text{entrada permutada} \to \text{output natural},
\]
o que permite que a multiplicação pontual ocorra no mesmo formato de dados, reduzindo reordenações explícitas.
