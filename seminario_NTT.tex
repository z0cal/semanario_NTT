...
É possivel provar que a DFT é uma transformação linear \ref{sec:linear}, logo, pode ser representada matricialmente.
...
\end{bmatrix}
\]
Percebe-se que nesse caso discreto, a DFT e IDFT atuam como uma matriz mudança de base, saindo da base do tempo e indo para base das raízes unitarias

Além disso, a DFT pode ser vista de outra óptica, como avalição de polinômios.
...
\emph{interpolar} (recuperar os coeficientes) a partir dessas avaliações. (Essa perspectiva baseia-se na determinição unívoca do polinómio de grau $N-1$ por N pontos vejá a seção~\ref{sec:Determinição unívoca do polinômio} para demonstração desse fato)


\section{ A Multiplicação de Polinômios e a Complexidade Computacional}

Um problema simplificado pela mudança de domino é a multiplicação de polinômios. Tome os polinômios $f(x)$ e $g(x)$ de grau $N-1$: 
...
O algoritmo da FFT foi redescoberto por Cooley e Tukey em 1965, uma vez que Gauss já tinha utilzado um algoritmo semelhante para calcular a órbitas de asteroides em 1805. 
...
As raízes unitárias possuem propriedades cíclicas e certas simetrias que permitem a economia nos cálculos, vejámos um exemplo.
...
\end{quote}
Álem de que, pela periodicidade $\zeta_N^{a+N}=\zeta_N^a$ e $\zeta_N^{2}=\zeta_{N/2}$, esta última pode ser vista na figura \ref{fig:comparacao_raizes}, onde $\zeta_8^2=\zeta_4$ 
...
Como a IDFT é praticamente identica à DFT, podemos usar o mesmo algoritmo da FFT para calcular a IFFT
\begin{algorithm}
...
Concluímos, portanto, que existe uma raiz primitiva $2N$-ésima da unidade em $\mathbb{Z}/p\mathbb{Z}$ se, e somente se, $2N\mid(p-1)$ (equivalentemente via teorema de Euler Totiente, $p\equiv 1 \pmod{2N}$).

\hrulefill

Dados esses critérios de de existência, trataremos da estrutura algébrica da transformada. A NTT Negacíclica é definida no anel quociente 
\[
R = \frac{\mathbb{Z}_p[x]}{(x^N+1)}
\]

ou Seja, há uma composição de operações modulares.(a imagem \ref{fig:torus} ilustra a sequencia das duas operações visualmente.)
...
Onde a matriz inversa $\mathbf{W}_N^{-1}$ é definida como o inverso modular das potências impares de $\psi$, escalada pelo fator $N^{-1}$:

\[
\mathbf{W}_N^{-1} = N^{-1}
\begin{bmatrix}
1 & 1 & 1 & \cdots & 1 \\
1 & \psi^{-3} & \psi^{-5} & \cdots & \psi^{-(2N-1)} \\
...
Definindo o \emph{twiddle factor} do nível como
\[
w_k = x_k = \psi^{2k+1},
\]
...
Y_o[k] &= 2^{-1}\bigl(Y[k]-Y[k+N/2]\bigr)\,w_k^{-1}\pmod p.
\end{aligned}}
\]
...
nos blocos correspondentes usando $\psi_{inv}^2$ (em completa simetria com o pseudocódigo).

\subsubsection*{4. Ordem das operações: CT vs GS}
A diferença estrutural entre as duas direções pode ser resumida assim:
\begin{enumerate}
    \item Na \textbf{Ida (CT / DIT)}, primeiro resolvemos recursivamente e \emph{depois} combinamos com o fator $w_k$.
    \item Na \textbf{Volta (GS)}, primeiro desfazemos a combinação local (somar/subtrair e multiplicar por $w_k^{-1}$)
...
\subsubsection*{5. Fluxo de dados e bit-reversal}

Em implementações \emph{iterativas in-place} (radix-2), a ordem em que os subproblemas são combinados não é apenas um detalhe:
ela determina \textbf{em quais posições de memória} cada coeficiente transformado vai parar ao final.
Esse fenômeno é normalmente descrito como uma \emph{permutação por bit-reversal} (reversão de bits).

\paragraph{O que é bit-reversal?}
Assuma $N = 2^s$. Para um índice $k \in \{0,\dots,N-1\}$, escreva sua expansão binária em $s$ bits:
\[
k = (b_{s-1}\, b_{s-2}\, \dots\, b_1\, b_0)_2.
\]
O índice \emph{bit-reverso} de $k$ é definido por
\[
\operatorname{rev}(k) = (b_0\, b_1\, \dots\, b_{s-2}\, b_{s-1})_2.
\]
A “saída permutada” a que nos referimos é tipicamente a saída na ordem $k \mapsto \operatorname{rev}(k)$.

\paragraph{Por que a permutação aparece no iterativo? (intuição de árvore)}
A decomposição radix-2 divide o problema em metades sucessivas.
No ponto de vista da árvore de recursão, cada nível “decide” um bit do índice (tempo ou frequência),
mas no iterativo in-place nós percorremos os níveis numa ordem fixa e reaproveitamos a memória.
O resultado é que os bits do índice acabam sendo fixados \emph{na ordem inversa} ao índice natural,
o que se manifesta como bit-reversal.

\paragraph{Exemplo pequeno ($N=8$)}
Aqui $s=3$. A tabela abaixo mostra a permutação:
\[
\begin{array}{c|c|c}
k & (k)_2 & \operatorname{rev}(k) \\
\hline
0 & 000 & 000 = 0 \\
1 & 001 & 100 = 4 \\
2 & 010 & 010 = 2 \\
...
\end{array}
\]
Portanto, “saída permutada” significa, por exemplo, que o valor que conceitualmente é $Y[1]$ pode acabar armazenado
na posição $4$, pois $\operatorname{rev}(1)=4$.

\paragraph{DIT vs.\ DIF: onde fica o bit-reversal?}
A presença (e o lado) do bit-reversal depende de duas escolhas:
(i) se você usa \emph{decimation-in-time} (DIT) ou \emph{decimation-in-frequency} (DIF),
e (ii) como você organiza os laços no iterativo in-place.
Em muitas bibliotecas e implementações de alto desempenho, escolhe-se deliberadamente um par de variantes
que \textbf{faz as permutações se cancelarem} no fluxo completo \emph{NTT $\to$ multiplicação ponto a ponto $\to$ INTT}.

\paragraph{O par “CT (DIT)” e “GS (DIF)” na prática}
Um arranjo muito comum (e útil para reduzir reordenações explícitas) é:
\[
\text{(DIT) CT}:\ \text{input natural} \ \to\  \text{saída em bit-reversal}
\qquad\text{e}\qquad
\text{(DIF) GS}:\ \text{entrada em bit-reversal} \ \to\  \text{output natural}.
\]
A vantagem é operacional:

\begin{itemize}
\item Você aplica a NTT (CT/DIT) em $a$ e em $b$, obtendo $A$ e $B$ \textbf{no mesmo formato permutado}.
\item A multiplicação ponto a ponto $C[i]=A[i]\cdot B[i]$ pode ser feita \textbf{diretamente}, pois as duas saídas
estão alinhadas pela \emph{mesma} permutação.
\item Em seguida, a INTT (GS/DIF) consome essa entrada permutada e devolve o vetor no \textbf{formato natural},
restaurando a ordem sem precisar de uma etapa explícita de bit-reversal.
\end{itemize}

Em outras palavras, a permutação não “some”: ela é \textbf{administrada} de modo que não vire custo extra no caminho crítico.
Se, por motivos de depuração/visualização, você quiser a saída natural já após a NTT,
pode-se aplicar uma permutação $i \leftrightarrow \operatorname{rev}(i)$ em $O(N)$,
mas em implementações rápidas costuma ser preferível \emph{evitar} esse passo e deixar a ordem permutada “propagar”
até a INTT.
...
\begin{algorithm}[H]
\caption{NTT negacíclica iterativa in-place (radix-2 CT / DIT) com saída bit-reversed}
\begin{algorithmic}[1]
\Function{NegacyclicNTT\_CT\_DIT}{$a[0..N-1],\, p,\, \psi$}
    \State \Comment{$N$ é potência de 2; $\psi$ tem ordem $2N$ em $\mathbb{Z}_p$}
    \Statex \Comment{Após o algoritmo: $a[\mathrm{rev}(k)] = X[k]$, onde $X[k]=\sum_{n} x[n]\psi^{(2k+1)n}$}
    
    \State \Comment{(1) Twist: $a[n] \leftarrow a[n]\cdot \psi^n$ (gera raízes ímpares)}
    \State $t \gets 1$
    \For{$n \gets 0$ \textbf{to} $N-1$}
        \State $a[n] \gets (a[n]\cdot t)\bmod p$
        \State $t \gets (t\cdot \psi)\bmod p$
    \EndFor

    \State \Comment{(2) NTT cíclica CT/DIT com raiz $\omega=\psi^2$ (ordem $N$)}
    \State $\omega \gets (\psi\cdot \psi)\bmod p$
    
    \For{$\ell \gets 2$ \textbf{to} $N$ \textbf{step} $\ell \gets 2\ell$}
        \State $h \gets \ell/2$
        \State $w_\ell \gets \omega^{\,N/\ell}\bmod p$ \Comment{raiz principal de ordem $\ell$}
        
        \For{$s \gets 0$ \textbf{to} $N-1$ \textbf{step} $\ell$}
            \State $w \gets 1$
            \For{$j \gets 0$ \textbf{to} $h-1$}
                \State $u \gets a[s+j]$
                \State $v \gets (a[s+j+h]\cdot w)\bmod p$
                \State $a[s+j] \gets (u+v)\bmod p$
                \State $a[s+j+h] \gets (u-v)\bmod p$
                \State $w \gets (w\cdot w_\ell)\bmod p$
            \EndFor
        \EndFor
    \EndFor
    
    \State \Return $a$ \Comment{$a[\mathrm{rev}(k)] = X[k]$}
\EndFunction
\end{algorithmic}
\end{algorithm}

\begin{algorithm}[H]
\caption{INTT negacíclica iterativa in-place (GS / DIF) compatível com NTT CT/DIT de saída bit-reversed}
\begin{algorithmic}[1]
\Function{NegacyclicINTT\_GS\_DIF}{$a[0..N-1],\, p,\, \psi$}
    \State \Comment{$N$ é potência de 2; $\psi$ tem ordem $2N$ em $\mathbb{Z}_p$}
    \Statex \Comment{Entrada esperada: $a[\mathrm{rev}(k)] = X[k]$, onde $X[k]=\sum_n x[n]\psi^{(2k+1)n}$}
    \Statex \Comment{Saída: $a[n]=x[n]$ em ordem natural}

    \State \Comment{(1) INTT cíclica GS/DIF de tamanho $N$ com raiz $\omega=\psi^2$}
    \State $\omega \gets (\psi\cdot \psi)\bmod p$
    \State $\omega^{-1} \gets \omega^{\,p-2}\bmod p$ \Comment{inverso modular (Fermat)}
    
    \For{$\ell \gets N$ \textbf{down to} $2$ \textbf{step} $\ell \gets \ell/2$}
        \State $h \gets \ell/2$
        \State $w_\ell \gets (\omega^{-1})^{\,N/\ell}\bmod p$ \Comment{raiz inversa de ordem $\ell$}
        
        \For{$s \gets 0$ \textbf{to} $N-1$ \textbf{step} $\ell$}
            \State $w \gets 1$
            \For{$j \gets 0$ \textbf{to} $h-1$}
                \State $u \gets a[s+j]$
                \State $v \gets a[s+j+h]$
                \State $a[s+j] \gets (u+v)\bmod p$
                \State $a[s+j+h] \gets ((u-v)\cdot w)\bmod p$
                \State $w \gets (w\cdot w_\ell)\bmod p$
            \EndFor
        \EndFor
    \EndFor

    \State \Comment{(2) Normalização: multiplica por $N^{-1}$}
    \State $N^{-1} \gets N^{\,p-2}\bmod p$
    \For{$n \gets 0$ \textbf{to} $N-1$}
        \State $a[n] \gets (a[n]\cdot N^{-1})\bmod p$
    \EndFor

    \State \Comment{(3) Untwist: $a[n] \leftarrow a[n]\cdot \psi^{-n}$}
    \State $\psi^{-1} \gets \psi^{\,p-2}\bmod p$
    \State $t \gets 1$
    \For{$n \gets 0$ \textbf{to} $N-1$}
        \State $a[n] \gets (a[n]\cdot t)\bmod p$
        \State $t \gets (t\cdot \psi^{-1})\bmod p$
    \EndFor
...
Nos algoritmos da FFT e da NTT, o n-ésimmo número de Fibonacci foi calculado pela seguinte relação:
\[\begin{bmatrix}
    F_{n+1}\\
    F_{n}
\end{bmatrix}
...
\end{bmatrix}
\]
sendo a multiplicação que ocorre tanto na exponenciação quanto no procuto interno foram otimizadas com o devido método.

Além disso, como o foco foi apenas em comparar a velocidade dos métodos, utilizou-se de scrpites para a geração de macros com os valores das raízes primitivas, raizes da unidade, entres outras constantes, para mais informações veja o repositório.


(A GMP é uma biblioteca otimizada para operações criptográficas)

\textbf{Setup:}

\textbf{CPU:} 13th Gen Intel(R) Core(TM) i7-1365U (12) @ 5.20 GHz

\textbf{GPU:} Intel UHD Graphics @ 1.30 GHz [Integrated]
\begin{center}
\begin{tabular}{|l|r|}
\hline
Algoritmo & n-ésimo nº de Fibonacci \\ \hline
Algoritmo Naive & 44 \\ \hline
Algoritmo iterado & 566'053 \\ \hline
...
\begin{figure}
    ...
    \caption{árvore de fracionameto da NTT }
